...
DEFAULT_OUT = r"C:\Users\zhouhuajian\Desktop\交付"
...
def build_lookup(sftp, remote_dir):
    ...
    目录不存在或无法访问时返回空，不阻塞主流程（主流程主要依赖 file_path 精确定位）。"""
    try:
        names = [n for n in sftp.listdir(remote_dir) if not n.startswith(".")]
    except IOError as e:
        print(f"[!] 远程目录不存在或无法访问: {remote_dir}")
        print(f"    {e}")
        print("[i] 将仅依赖 JSON 里的 file_path + --root 精确定位图片，请确保 --root 正确。")
        return {}, set()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--remote-dir", default="/opt/ad_mining/workspace/projects/dd/images",
