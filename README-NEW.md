...
> 本文档对应**新版** `app.py`（端口 **8009**）与配套 `前端.html`（暗色科幻风）。
> 若目录内 `app.py` 仍为旧版（8008/CLIP），请以实际代码为准；下文按新版设计描述。

基于 **SigLIP + Grounding DINO-Base + YOLOv8x** 三模型协同的**自动驾驶场景挖掘终端**：
支持**多项目物理隔离**、自动即时向量化入库、文本搜图/以图搜图、高维特征去重、开放词汇目标预标与标注导出。
后端 FastAPI，前端为纯原生 HTML/JS 单页（由后端托管）。
...
1. Google **SigLIP-SO400M**（`google/siglip-so400m-patch14-384`）
2. **Grounding DINO-Base**（`IDEA-Research/grounding-dino-base`）
3. **YOLOv8x**（本地 `yolov8x.pt`）
...
冗余可导出 ID 清单（TXT），或 `POST /api/delete_and_sync` 物理粉碎冗余图片并**重建索引同步**。
...
- `exportData(checked=true/false)`：导出当前页勾选/未勾选的卡片 JSON
  `{id, filename, url, score, annotations{boxes/labels/scores}, userTags}`。
...
- 前端引用的 `/static_root/TT.jpg` 与目录内 `TT.png` 资源名可能不一致——若页面加载静态资源异常，请将 logo 文件命名为 `TT.jpg` 或调整 `前端.html` 中的引用路径。
